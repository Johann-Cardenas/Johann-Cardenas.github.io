/* ============================================================
   ...
   2. MILLIMETERS, NOT METERS.
      glTF's convention is meters, and this deliberately departs
      from it. Millimeters is the app's canonical unit and the one
...
/**
 * Render frame -> engineering frame, and meters -> millimeters.
    ...
    assembly.root.traverse((o) => {
        ...
        if (any.isInstancedMesh) {
            for (let i = 0; i < any.count; i++) {
                ...
                // references it from N nodes, instead of 34 copies of a tire.
                ...
                // before a translation already expressed in millimeters, and
        ...
        new GLTFExporter().parse(
            ...
            (err) => reject(new Error(`glTF export failed: ${err?.message ?? err}`)),
            {
                ...
                extras: {
                    ...
                    note: 'Millimeters, not the glTF meter convention — deliberately, so this '
...
export function exportOBJ(viewport, opts = {}) {
    ...
    const header = [
        ...
        '#',
        '# OBJ carries no instancing, so every tire is a full copy of its geometry.',
